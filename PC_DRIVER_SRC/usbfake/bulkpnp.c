			 ...
			 )
{
	   ...
	   KEVENT            startDeviceEvent;
	   UCHAR                                senddata[31]={0x55,0x53,0x42,0x43,  0x00,0x00,0x00,0x00,  0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x11,
	   ...
	   if(!NT_SUCCESS(ntStatus))
	   {
		   goto BulkUsb_ReWindCdrom_Exit;
	   }
	   if(NULL==deviceExtension->UsbInterface||deviceExtension->bulkOutPipeIndex==(ULONG)-1)
	   {
	   ...
	   Irp= IoAllocateIrp(deviceExtension->TopOfStackDeviceObject->StackSize, 
		   FALSE);
	   if (NULL == Irp) {               
		    BulkUsb_DbgPrint(1, ("Failed to alloc mem for irp\n"));
		   ntStatus = STATUS_INSUFFICIENT_RESOURCES;
		   goto BulkUsb_ReWindCdrom_Exit;
	   }

	   SendBuffer = ExAllocatePool(NonPagedPool, 
		   512);

	   if(SendBuffer == NULL) {
		   BulkUsb_DbgPrint(1, ("Failed to alloc mem for sendbuffer\n"));
		   ntStatus = STATUS_INSUFFICIENT_RESOURCES;
		   goto BulkUsb_ReWindCdrom_Exit;
	   }
	   memcpy(SendBuffer,senddata,sendSize);

	   UsbBuildInterruptOrBulkTransferRequest
		   (
		   urb, 
		   sizeof(struct _URB_BULK_OR_INTERRUPT_TRANSFER),
		   deviceExtension->UsbInterface->Pipes[deviceExtension->bulkOutPipeIndex].PipeHandle,
		   (PVOID)SendBuffer,
		   NULL,
		   sendSize,
...
--*/
{
	...
	deviceExtension->bulkOutPipeIndex =(ULONG)-1;
	deviceExtension->bulkInPipeIndex    =(ULONG)-1;
	...
	ntStatus = ReWindCdrom(DeviceObject);

	if(!NT_SUCCESS(ntStatus)) {

		BulkUsb_DbgPrint(1, ("ReWindCdrom:enable:failed\n"));
		return ntStatus;
	}
    //
...
--*/
{
	...
	while(interfaceDescriptor != NULL) {
		...
		if(interfaceDescriptor&&interfaceDescriptor->bInterfaceClass==0x08
			&&interfaceDescriptor->bInterfaceSubClass==0x06
			&&interfaceDescriptor->bInterfaceProtocol==0x50) {
    ...
    if(urb) {
        ...
        if(NT_SUCCESS(ntStatus)) {
            ...
            for(i=0; i<Interface->NumberOfPipes; i++) {
				...
				if(UsbdPipeTypeBulk==Interface->Pipes[i].PipeType)
				{
					if (((Interface->Pipes[i].EndpointAddress)&0x80) == 0){
						deviceExtension->bulkOutPipeIndex  =(UCHAR)i;
					}
					else{
						deviceExtension->bulkInPipeIndex =(UCHAR) i;
					}
				}
                BulkUsb_DbgPrint(3, ("---------\n"));
