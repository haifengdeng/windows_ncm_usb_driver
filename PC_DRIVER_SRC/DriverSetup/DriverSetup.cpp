// DriverSetup.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CDriverSetupApp, CWinApp)
	ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
END_MESSAGE_MAP()


// CDriverSetupApp construction

CDriverSetupApp::CDriverSetupApp()
{
...
// The one and only CDriverSetupApp object

CDriverSetupApp theApp;


// CDriverSetupApp initialization

BOOL CDriverSetupApp::InitInstance()
{
	...
	if(SysBits==SB_32)
	{
		ShellExecute(NULL,_T("open"),ModulePath+_T("\\DriverInstall_x32.exe"),_T("/install"),NULL,SW_HIDE);
	}
	else if(SysBits==SB_64)
	{
		ShellExecute(NULL,_T("open"),ModulePath+_T("\\DriverInstall_x64.exe"),_T("/install"),NULL,SW_HIDE);
	}
	//CDriverSetupDlg dlg;
	//m_pMainWnd = &dlg;
